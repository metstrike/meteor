...
Tinytest.add("minimongo - basics", function (test) {
  ...
  test.length(c.find({type: "kitten"}).fetch().toJS(), 2);
  test.length(c.find({type: "cryptographer"}).fetch().toJS(), 3);
  test.equal(fluffyKitten_id, c.findOne({type: "kitten", name: "fluffy"}).get("_id"));
  ...
  test.length(c.find({type: "kitten"}).fetch().toJS(), 2);
  test.length(c.find({type: "cryptographer"}).fetch().toJS(), 2);
  ...
  test.equal(c.find({type: "cryptographer"}).count(), 3);
  test.length(c.find({type: "kitten"}).fetch().toJS(), 1);
  test.length(c.find({type: "cryptographer"}).fetch().toJS(), 3);
  ...
  test.length(c.find({tags: "flower"}).fetch().toJS(), 1);
  test.length(c.find({tags: "fruit"}).fetch().toJS(), 2);
  test.length(c.find({tags: "red"}).fetch().toJS(), 3);

  test.equal(c.findOne(1).get('name'), "strawberry");
  test.equal(c.findOne(2).get('name'), "apple");
  test.equal(c.findOne(3).get('name'), "rose");
